...
async function populate() {
    ...
    const url = "https://omasta5.github.io/JavaScript-Week11/js/icecream.json";
...
function populaeHeader(jsonBody) {
...
function showTopFlavours(jsonBody) {
    ...
    for(let i = 0; i < topFlavours.length; i++) {
        ...
        p2.textContent = `Type: ${topFlavours[i].calories}`;
        ...
        // <li></li>
        // </ul>
        let ingrediants = topFlavours[i].ingrediants;
        for(let j = 0; j < ingrediants.length; j++) {
            console.log(ingrediants[j]);
            let listItem = document.createElement("li"); // <li></li>
            listItem.textContent = ingrediants[j];
